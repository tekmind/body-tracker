...
- **(on hold, 8/7/26 — owner said hold off)** Consider a WHOOP integration for what its API *does* carry — energy burned, strain, recovery, resting HR, sleep, and workout sessions. Would mirror the Withings files exactly. Three things to know before starting:
...
- **Urinalysis markers** — 8/7/26. `resolveMarker` now takes the panel name;
  ...
  `urine_*` markers in a Urinalysis category instead of joining the serum
  trend lines they were silently corrupting (urine "Neg" rows were inflating
  serum glucose, bilirubin and WBC counts). Existing rows re-keyed by script.
- **`(calc)` unit strip** — 8/7/26. `api/lab-parse.js` now cleans the unit
  ("mg/dL (calc)" → "mg/dL") and drops a ref_text that is only a bracketed
  unit rather than a range. 18 stored rows backfilled by script.
- **Backup Download button** — 8/7/26. "Download export" sits next to Export
  and saves the in-app blobs directly, no Export click first.
- ~~Spoonacular as a fourth food source~~ — dropped 8/7/26; owner is happy
  with the food-add workflow as is.
