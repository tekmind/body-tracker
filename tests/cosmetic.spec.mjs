...
for (const [w, h, tag] of [[390, 780, "phone"], [1200, 900, "desktop"]]) {
  ...
  const ref = await p.evaluate(() => {
    ...
    return {
      ...
      clipped: t.top < plot.top,
      offCentre: Math.round(Math.abs((t.left + t.right) / 2 - (plot.left + plot.right) / 2)),
  ...
  check(`${tag}: it isn't clipped by the top of the chart`, !ref.clipped);
  check(`${tag}: it's centred over the line`, ref.offCentre <= ref.plotW * 0.08, `${ref.offCentre}px off centre`);
